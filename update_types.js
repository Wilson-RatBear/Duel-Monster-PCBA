...
let content = fs.readFileSync('packages/game-types/src/index.ts', 'utf8');
...
content = content.replace(/area:\s*'([^']+)'/g, (match, p1) => {
    return `area: '${p1}', academicMetadata: { careerArea: '${p1}' as CareerArea, academicConcept: 'Concepto General' }`;
});
...
content = content.replace(/export const MONSTERS: MonsterCard\[\] = \[([\s\S]*?)\];/, (match, p1) => {
    return "export const MONSTERS: MonsterCard[] = [" + p1 + "," + new_monsters + "];";
});

content = content.replace(/export const SPELLS: SpellCard\[\] = \[([\s\S]*?)\];/, (match, p1) => {
    return "export const SPELLS: SpellCard[] = [" + p1 + "," + new_spells + "];";
});

fs.writeFileSync('packages/game-types/src/index.ts', content, 'utf8');
