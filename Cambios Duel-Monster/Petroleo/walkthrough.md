# Walkthrough: Arquitectura Ingeniería de Petróleo (Semestre I)

¡Implementación completada! He añadido exitosamente el soporte técnico en el servidor y en los tipos base para permitir que las cartas operen bajo leyes espaciales y de conservación de la materia.

## Cambios Realizados

1. **Tipos Ampliados y Geometría (`packages/game-types/src/index.ts`)**
   - ```diff:index.ts
...
export interface MonsterCard extends BaseCard {
  ...
  specialAttackCost: number;
  hasAttacked?: boolean;
  position?: PositionR2; // Geometría Analítica
...
export interface ClientToServerEvents {
  ...
  setName: (name: string) => void;
  setReady: (ready: boolean) => void;
  summonMonster: (cardId: string, position?: PositionR2) => void;
  castSpell: (cardId: string) => void;
  attackBasic: () => void;
  attackSpecial: () => void;
  endTurn: () => void;
...
```
   - Añadí `PETROLEO` a la lista de áreas formativas.
   - Expandí `MonsterCard` con las propiedades opcionales `position` (coordenadas R2), `spatialRange` y `stoichiometricCost`. Esto convierte a tu tablero tradicional en un plano cartesiano virtual bidimensional.

2. **Controladores de Geometría y Química (`apps/server/src/features/duel/duel.socket.ts`)**
   - ```diff:duel.socket.ts
...
export function setupDuelSocket(io: Server<ClientToServerEvents, ServerToClientEvents>) {
  io.on('connection', (socket) => {
    ...
    socket.on('summonMonster', (cardId, position) => {
      ...
      // Control Espacial (Geometría Analítica)
      monsterToSummon.position = position || { x: 0, y: 0 };
...
```
   - **Balance Estequiométrico:** En el evento `summonMonster`, el servidor verifica si la carta exige un balance. Si es así, la invocación fallará si el jugador no tiene suficientes recursos, obligándolo a sacrificar cartas de su mano equivalentes a la "masa" de invocación, emulando la conservación.
   - **Geometría en Batalla:** En el evento `attackBasic`, antes de computar el daño matemático, el servidor aplica la fórmula euclidiana de la distancia: $\sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2}$. Si los monstruos se encuentran más lejos que el rango de alcance de la carta atacante, el ataque se deniega y se emite un error explicando la distancia fallida al jugador.

## Consideraciones Finales

> [!TIP]
> Tu motor de juego es ahora uno de los más avanzados didácticamente. Te recomiendo que cuando desarrolles la Interfaz de Usuario (UI) en Next.js, utilices un tablero de grilla o proporciones controles visuales (Drag & Drop) que envíen el evento `summonMonster(cardId, { x, y })` al socket para que el usuario pueda visualizar la geometría.
