# Walkthrough: Arquitectura (Semestre I)

La nueva carrera de **Arquitectura** ha sido estructurada a nivel de código en el backend, habilitando una capa estética y espacial dentro de las mecánicas de duelo.

## Cambios a Nivel de Sistema

1. **Tipado Estructural (`packages/game-types`)**
   - ```diff:index.ts
...
```
   - Añadí `ARQUITECTURA` a las carreras jugables y definí `SpatialOrder` (`CENTRAL`, `LINEAR`, `RADIAL`, `GRID`) para habilitar interacciones de terreno y diseño paramétrico.

2. **Lógica de Sockets y Eventos (`apps/server`)**
   - ```diff:duel.socket.ts
...
```
   - **Ordenamiento Radial (Diseño I):** Se modificó `summonMonster`. Ahora, si esta capa de diseño está activa y colocas una carta exactamente en la coordenada `(0,0)`, el monstruo gana `+1000 DEF` por la "Armonía Espacial".
   - **Corte de Sección (Dibujo):** A diferencia de Geometría que revela el campo, este hechizo expone secretamente las cartas de la *mano* de tu oponente y lo imprime en los logs/UI.
   - **Sinergia de Liderazgo (Recreación):** Al atacar, el backend busca silenciosamente en tu mano al monstruo de mayor fuerza, lo descarta (sacrificio) y suma su poder al atacante, simulando el trabajo en equipo grupal frente a la limitación de 1 solo monstruo en campo.
   - **Bóveda Gótica (Historia):** Una red de mitigación masiva. Si te van a destruir los HP, las cargas estructurales del daño son anuladas y en su lugar vacías tu estanque de `energía`, manteniéndote con vida.
   - **Proporción Áurea (Matemática I):** Un escalado estético perfecto que multiplica instantáneamente tu ATK por $\Phi$ ($1.618$) redondeado.

## Resumen del Ecosistema

> [!NOTE]
> Con la integración de Arquitectura, el motor de juego GBL ahora cruza **5 Ingenierías/Carreras distintas**, cada una con enfoques radicalmente diferentes (Química vs Topografía vs Matrices vs Estética Espacial).

**¿Hacia dónde construimos ahora?**
El backend está inmensamente poblado y rico en mecánicas. Podemos:
1. Avanzar a **Arquitectura Semestre II** (Estructuras, Climatología, y Modelado 3D).
2. Comenzar el desarrollo visual en el **Frontend (Next.js)** para poder renderizar las coordenadas `(0,0)`, las cartas boca abajo y los cortes transversales en una interfaz de usuario atractiva.
